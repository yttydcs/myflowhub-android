...
func TestTopicBusEventBuffer_RingAndPull(t *testing.T) {
	b := newTopicBusEventBuffer(3)

	s1 := b.append(topicBusEvent{Topic: "t1"})
	s2 := b.append(topicBusEvent{Topic: "t2"})
	s3 := b.append(topicBusEvent{Topic: "t3"})
	if s1 != 1 || s2 != 2 || s3 != 3 {
	...
	s4 := b.append(topicBusEvent{Topic: "t4"})
	if s4 != 4 {
