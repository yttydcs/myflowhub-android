...
// 本文件覆盖 Android `hubmobile` 桥接中与 `varstore_events` 相关的行为。

import "testing"

func TestVarStoreEventBuffer_RingAndPull(t *testing.T) {
	b := newVarStoreEventBuffer(3)

	s1 := b.append(varStoreEvent{Action: "a"})
	s2 := b.append(varStoreEvent{Action: "b"})
	s3 := b.append(varStoreEvent{Action: "c"})
	if s1 != 1 || s2 != 2 || s3 != 3 {
	...
	s4 := b.append(varStoreEvent{Action: "d"})
	if s4 != 4 {
