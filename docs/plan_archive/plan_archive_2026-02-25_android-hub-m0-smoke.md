# Plan Archive - Android：Hub（M0）冒烟补齐（LAN 直连 + Parent 链路）

> 归档说明：
> - 本文档为历史 workflow `android-hub-m0-smoke` 的 `plan.md` 归档副本。
> - 归档目的：保留可审计的执行计划，避免与后续 workflow 的 `plan.md` 混用。
> - 对应实现/验收归档：`docs/change/2026-02-25_android-hub-m0-smoke.md`
